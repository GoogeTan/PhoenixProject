...
class EndDimension(worldIn: World, typeIn: DimensionType) : Dimension(worldIn, typeIn, 0.0f)
{
    private val spawn = BlockPos(100, 50, 0)
    lateinit var biomeProvider : NewEndBiomeProvider
    var SPAWN = BlockPos(100, 50, 0)
    var dragonFightManager: CustomDragonFightManager? = if (worldIn is ServerWorld) CustomDragonFightManager(worldIn, worldIn.worldInfo.getDimensionData(typeIn).getCompound("DragonFight"), this) else null
    ...
    @OnlyIn(Dist.CLIENT) override fun calcSunriseSunsetColors(celestialAngle: Float, partialTicks: Float) = null
    @OnlyIn(Dist.CLIENT) override fun isSkyColored()   = false
    @OnlyIn(Dist.CLIENT) override fun getCloudHeight() = 8.0f
    @OnlyIn(Dist.CLIENT) override fun doesXZShowFog(x: Int, z: Int) = false
    @OnlyIn(Dist.CLIENT) override fun getFogColor(celestialAngle: Float, partialTicks: Float) = Vec3d(0.09411765, 0.07529412, 0.09411765)
    ...
    override fun onWorldSave()
    {
        ...
        {
            nbt.put("DragonFight", dragonFightManager?.write()?:CompoundNBT())
        }
