...
final class EnderDragonEntity(type: EntityType<out EnderDragonEntity>, worldIn: World) :
    MobEntity(EntityType.ENDER_DRAGON, worldIn), IMob
{
    private val ringBuffer = Array(64) { DoubleArray(3) }
    var ringBufferIndex = -1
    private val dragonParts: Array<EnderDragonPartEntity>
    private val dragonPartHead:      EnderDragonPartEntity = EnderDragonPartEntity(this, "head", 1.0f, 1.0f)
    private val dragonPartNeck:      EnderDragonPartEntity = EnderDragonPartEntity(this, "neck", 3.0f, 3.0f)
    private val dragonPartBody:      EnderDragonPartEntity = EnderDragonPartEntity(this, "body", 5.0f, 3.0f)
    private val dragonPartTail1:     EnderDragonPartEntity = EnderDragonPartEntity(this, "tail", 2.0f, 2.0f)
    private val dragonPartTail2:     EnderDragonPartEntity = EnderDragonPartEntity(this, "tail", 2.0f, 2.0f)
    private val dragonPartTail3:     EnderDragonPartEntity = EnderDragonPartEntity(this, "tail", 2.0f, 2.0f)
    private val dragonPartRightWing: EnderDragonPartEntity = EnderDragonPartEntity(this, "wing", 4.0f, 2.0f)
    private val dragonPartLeftWing:  EnderDragonPartEntity = EnderDragonPartEntity(this, "wing", 4.0f, 2.0f)
    var prevAnimTime = 0f
    var animTime = 0f
    var slowed = false
    var deathTicks = 0
    private var yawFactor = 0f
    var closestEnderCrystal: EnderCrystalEntity? = null
    var fightManager: CustomDragonFightManager? = null
    private val phaseManager: CustomPhaseManager
    private var growlTime = 100
    private var sittingDamageReceived = 0
    private val pathPoints = arrayOfNulls<PathPoint>(24)
    private val neighbors = IntArray(24)
    private val pathFindQueue = PathHeap()
    ...
    {
        super.registerData()
        getDataManager().register(PHASE, PhaseType.HOVER.id)
    }
            ...
            } else
            {
                ...
                } else
                {
                    ...
                    if (vec3d != null)
                    {
                        ...
                        yawFactor *= 0.8f
                        yawFactor = (yawFactor.toDouble() + d5 * iphase.yawFactor.toDouble()).toFloat()
                        rotationYaw += yawFactor * 0.1f
                ...
                setPosition(dragonPartBody, (f3 * 0.5f).toDouble(), 0.0, (-f18 * 0.5f).toDouble())
                setPosition(dragonPartRightWing, (f18 * 4.5f).toDouble(), 2.0, (f3 * 4.5f).toDouble())
                setPosition(dragonPartLeftWing, (f18 * -4.5f).toDouble(), 2.0, (f3 * -4.5f).toDouble())
                ...
                val f4 = MathHelper.sin(rotationYaw * (Math.PI.toFloat() / 180f) - yawFactor * 0.01f)
                val f19 = MathHelper.cos(rotationYaw * (Math.PI.toFloat() / 180f) - yawFactor * 0.01f)
                ...
                for (k in 0..2)
                {
                    var enderdragonpartentity: EnderDragonPartEntity? = null
                    if (k == 0)
                    {
                    ...
                    val offset = getMovementOffsets(12 + k * 2, 1.0f)
                    val f7 =
                        rotationYaw * (Math.PI.toFloat() / 180f) + simplifyAngle(offset[0] - adouble[0]) * (Math.PI.toFloat() / 180f)
                    ...
                    setPosition(
                        ...
                        offset[1] - adouble[1] - ((f23 + 1.5f) * f2).toDouble() + 1.5,
    ...
    private fun setPosition(
        p_226526_1_: EnderDragonPartEntity?,
        p_226526_2_: Double,
        p_226526_4_: Double,
        p_226526_6_: Double
    )
    ...
    private fun updateDragonEnderCrystal()
    {
        ...
        if (rand.nextInt(10) == 0)
        {
            ...
            for (crystal in list)
            {
                val d1 = crystal.getDistanceSq(this)
                if (d1 < d0)
                {
                    d0 = d1
     ...
     */
    private fun collideWithEntities(entities: List<Entity>)
    {
     ...
     */
    private fun simplifyAngle(angle: Double): Float
    {
        return MathHelper.wrapDegrees(angle).toFloat()
    }

    /**
     * Destroys all blocks that aren't associated with 'The End' inside the given bounding box.
     */
    private fun destroyBlocksInAABB(box: AxisAlignedBB): Boolean
    {
        ...
        val i1 = MathHelper.floor(box.maxY)
        val j1 = MathHelper.floor(box.maxZ)
        var flag = false
        var flag1 = false
        for (k1 in i..l)
        {
        ...
        if (flag1)
        {
            val blockpos1 = BlockPos(i + rand.nextInt(l - i + 1), j + rand.nextInt(i1 - j + 1), k + rand.nextInt(j1 - k + 1))
            world.playEvent(2008, blockpos1, 0)
        }
        return flag
    }

    fun func_213403_a(partEntity: EnderDragonPartEntity, source: DamageSource, countIn: Float): Boolean
    {
        var count = countIn
        ...
        {
            count = phaseManager.currentPhase!!.func_221113_a(source, count)
            if (partEntity !== dragonPartHead)
            {
                count = count / 4.0f + Math.min(count, 1.0f)
            }
            if (count < 0.01f)
            {
                false
     ...
     */
    private fun getNearestPpIdx(x: Double, y: Double, z: Double): Int
    {
        var f = 10000.0f
        var i = 0
    ...
    override fun writeAdditional(compound: CompoundNBT)
    {
        ...
        compound.putInt("DragonPhase", phaseManager.currentPhase!!.type.id)
    }
    ...
    fun getHeadPartYOffset(p_184667_1_: Int, p_184667_2_: DoubleArray, p_184667_3_: DoubleArray): Float
    {
        ...
        val d0: Double
        d0 = if (phasetype !== PhaseType.LANDING && phasetype !== PhaseType.TAKEOFF)
        {
    ...
    companion object
    {
        ...
        val PHASE = EntityDataManager.createKey(EnderDragonEntity::class.java, DataSerializers.VARINT)
    ...
    init
    {
        ...
        {
            (worldIn.dimension as EndDimension).getDragonFightManager()
        } else
        {
            null
        }
        phaseManager = CustomPhaseManager(this)
    }
}
