...
{
    val chapters = ArrayList<Pair<Int, Date>>()
...
class SaveHandler : IStorage<IChapterReader>
{
    ...
    {
        var nbt = (nbtIn as CompoundNBT)
        var count = nbt.getInt("count")
        for (i in 0 until count)
        {
            var id   = nbt.getInt("chid$i")
            var min  = nbt.getInt("chmin$i")
            var day  = nbt.getInt("chday$i")
            var year = nbt.getInt("chyear$i")
